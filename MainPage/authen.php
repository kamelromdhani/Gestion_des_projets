...
$req=$bdd->prepare('SELECT * FROM user WHERE login =? AND m2p =?   ');
$req->execute(array($_SESSION['login'],$_SESSION['pass']));
    ...
    <div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <ul class="nav navbar-right top-nav">
                        ...
                        <li class="message-preview">
                            <a href="#">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        <h5 class="media-heading"><strong>ramzi rezgui</strong>
                                        </h5>
                                        <p class="small text-muted"><i class="fa fa-clock-o"></i> Yesterday at 4:32 PM</p>
                                        <p>Lorem ipsum dolor sit amet, consectetur...</p>
                                    </div>
                ...
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-user"></i> <?php if(isset($_SESSION['nom'])){echo $_SESSION['nom'];} ; echo ' ';echo $_SESSION['prenom'];?> <b class="caret"></b></a>
		...
		<form name="f1" method="post" action=''> 
            <input type="text" class="form-control" VALUE="<?php /* afficher variable dans input */if(isset($_SESSION['cle'])){echo $_SESSION['cle'];}?>"  name="cle"/>					
            <button class="btn btn-default"  onclick="f1.action='projetconsp.php'" type="submit" name="serchbu" id="searchbu"><i class="glyphicon glyphicon-search"></i></button>
        </form>  
        </div>
		<div class="container">
  <h2>Result :</h2>
...
$req=$bdd->prepare('SELECT id_projet,name,manager,code,report_date,SchedualList,CostList,
QualityList FROM projects WHERE
name =? OR manager =? OR code =? OR report_date =? OR SchedualList =? OR CostList =? OR QualityList =? optionsRadios=? or direction =?  ');
$req->execute(array($_POST['cle'],$_POST['cle'],$_POST['cle'],$_POST['cle'],$_POST['cle'],$_POST['cle'],$_POST['cle'],$_POST['cle'],$_POST['cle']));
while ($donnees=$req->fetch())
{
...
//<a href='#' onclick='document.getElementById("test").submit()'>Ton lien</a>
	?>
      ...
      <tr>
        <td>
		<form id="test" method="post" action="projetconsaff.php">
           <input type="hidden"    name="id" id="id" value="<?php echo  $donnees['id_projet'];?>"/>
		   <input type="submit" class="btn btn-primary btn-lg" value="consult" >
        </form>
		</td>
		<td>
		<!--<a href='#'  onclick='document.getElementById("test").submit()' >--> <?php /* afficher variable dans input*/ echo $donnees['name'];?></a></td>
        <td><?php /* afficher variable dans input*/ echo $donnees['manager'];?></td>
        <td><?php /* afficher variable dans input*/ echo $donnees['code'];?></td>
...
$req=$bdd->query('SELECT id_projet,name,manager,code FROM projects ');
while ($donnees=$req->fetch())
{
	//<a href="forms.html">consult a project</a><a href='#' onclick='document.getElementById("test").submit()'>Ton lien</a>
	//<form id="test" action="le_lien_vers_ta_page">
